...
$outcomes = @get_records_sql_array(
	'SELECT o.id,o.outcome_name, o.degree_id
		FROM {outcomes} o
...
foreach ($outcomes as $outcome) {
	...
	if($outcome->degree_id){
		$degree_rec = get_record('degree_programs','id',$outcome->degree_id);
		$val = $degree_rec->degree_type . '-';
	}
	$options[$outcome->id] = array(
        'value'    => $val . $outcome->outcome_name,
        'disabled' => $disable,
    );
}
