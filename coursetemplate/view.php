...
if ($role) {
   ...
   // $viewdata = View::get_sharedviews_data(null, 0, $group->id);
	  $viewdata = View::get_sharedviews_data(null, 0, $group->id);//Eshwari-courseoutcome 
   // $smarty->assign('sharedviews', $viewdata->data);
	  $smarty->assign('sharedviews', $viewdata->data);//Eshwari-courseoutcome 
...
$outrec = get_record('outcomes','id',$group->outcome);
	  ...
	  */
//start- Eshwari
//$summativeViews = View::get_submitted_views_for_courseoutcome($group->id, $group->outcome);
    $outcourserec = get_record('courseoutcomes','id',$group->courseoutcome);
	  if($outcourserec->special_access){
		$viewdata = get_records_sql_array('
            SELECT v.id, v.title, v.description, v.owner, v.ownerformat, v.group, v.institution, v.assessCount
            FROM {view} v
		INNER JOIN {usr} u ON (u.id = v.owner)
		...
		AND u.primary_focus = ?
            ORDER BY title, id',
            array($group->id, $outcourserec->special_access)
        	);
	  }else{
        	$viewdata = get_records_sql_array('
            SELECT id, title, description, owner, ownerformat, "group", institution, assessCount
