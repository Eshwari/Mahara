...
define('INTERNAL', 1);
define('MENUITEM', 'coursetemplates/createcourseoutcomes');
...
$courses = array();
$courses['Course'] = 'Select a course template';
foreach($degcourses as $course){
	$courses[$course->id] = $course->coursetemplate_name;
}
...
if($courseoutcome_id == 0){
	...
	$disable = 1;
	//$default = $courseoutcomedes->coursetemplate_id;
}
...
global $courseoutcome_id;
   ...
   if($values['name'] == ''){
    ...
    if (get_field('courseoutcomes', 'id', 'coursetemplate_id', $values['degree'],'courseoutcome_name', $values['name'])) {
...
if($courseoutcome_id == 0){
	...
	$degree = $courseoutcomedes->coursetemplate_id;
}
...
db_begin();
$newcourseoutcome = insert_record('courseoutcomes', (object)array('courseoutcome_name' => $_POST['name'],'description' => $str, 'coursetemplate_id' => $degree, 'eval_type' => $_POST['assesstype'], 'main_courseoutcome' => $_POST['courseoutcomeid'], 'deleted' => 0),'id',true);
db_commit();
