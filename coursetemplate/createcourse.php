...
define('INTERNAL', 1);
define('MENUITEM', 'coursetemplate/groupsiown');
...
foreach ($courseoutcomes as $courseoutcome) {
	...
	$options[$courseoutcome->id] = array(
        'value'    => $courseoutcome->courseoutcome_name,
        'disabled' => $disable,
    );
}
...
else
{
	$createcoursegroup = pieform(array(
		...
		'elements' => array(
			...
			'groupcourseoutcome' => array(
					'type' => 'select',
					'title' => 'Associate the group to an courseoutcome',
					'collapseifoneoption' => false,
					'options' => $options,
			),
			'public' => array(
...
$smarty->display('coursetemplate/createcourse.tpl');
...
function createcoursegroup_cancel_submit() {
    redirect('/createtemplate/mygroups.php');
}
...
function createcoursegroup_submit(Pieform $form, $values) {
    ...
    redirect('/coursetemplate/viewcourse.php?id=' . $id);
}
