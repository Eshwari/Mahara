...
{if $sharedviews2}
    ...
    <p>
    <table class="fullwidth">
    {foreach from=$sharedviews2 item=view}
        <tr class="{cycle values='r0,r1'}">
            <td>
                <a href="{$WWWROOT}view/view.php?id={$view.id}">{$view.title|escape}</a>
                {if $view.sharedby}
        ...
        <tr class="{cycle values='r0,r1'}">
		    ...
		    {/if}
                ...
                {if $view.sharedby}
                    {str tag=by section=view}
                    {if $view.group}
                        <a href="{$WWWROOT}group/view.php?id={$view.group}">{$view.sharedby}</a>{if $view.formativecount>1} - Resubmitted{/if}
                    {elseif $view.owner}
                        <a href="{$WWWROOT}user/view.php?id={$view.owner}">{$view.sharedby}</a>{if $view.formativecount>1} - Resubmitted{/if}
                    {else}
                        {$view.sharedby}
                    {/if}
                {/if}				
...
{if $submittedviews2}
    ...
    <p>
    <table class="fullwidth">
    {foreach from=$submittedviews2 item=view}
	{if $view.status<2 && $view.evaluated==0}
        <tr class="{cycle values='r0,r1'}">
            <td>
		    {if $subgroup}
...
{if $finalviews}
    <h4>{str tag="viewsfinal" section="view"}</h4>
    <p>
    <table class="fullwidth">
