...
define('PUBLIC', 1);
define('INTERNAL', 1);
define('MENUITEM', 'courseoffering/members');
require(dirname(dirname(__FILE__)) . '/init.php');
require_once('courseoffering_group.php');
require_once('searchlib.php');
require_once(get_config('docroot') . 'interaction/lib.php');

define('COURSEOFFERING', param_integer('id'));
$membershiptype = param_alpha('membershiptype', null);

$courseoffering = courseoffering_current_courseoffering();
if (!is_logged_in() && !$courseoffering->public) {
    throw new AccessDeniedException();
}

define('TITLE', $courseoffering->courseoffering_name . ' - ' . get_string('Members', 'courseoffering'));

$role = group_user_access($courseoffering->id);

//Anusha
if ($group->outcome && $role == "member") {
	throw new AccessDeniedException();
}
//Anusha
//Eshwari
