...
class View {
	...
	private $self_describe_formative;
	//Start by Shashank
	...
	//Start-Eshwari
	 public static function viewcourse_search($query=null, $ownerquery=null, $ownedby=null, $copyableby=null, $limit=null, $offset=0, $extra=true, $sort=null) {
        ...
        $viewdata = get_records_sql_array('
            SELECT * FROM (
                SELECT
                    v.id, v.title, v.description, v.owner, v.ownerformat, v.group, v.institution, v.template, v.mtime, v.submittedcourseoutcome
                ' . $from . $where . '
                GROUP BY v.id, v.title, v.description, v.owner, v.ownerformat, v.group, v.institution, v.template, v.mtime, v.submittedcourseoutcome
            ) a
            ORDER BY a.' . $orderby . ', a.id ASC',
            $ph, $offset, $limit
        );
        ...
        if ($viewdata) {
            if ($extra) {
                View::get_extra_courseview_info($viewdata);
            }
        }
        else {
            $viewdata = array();
...
//start-Eshwari 
    public static function get_sharedcourseviews_data($limit=10, $offset=0, $groupid) {
        global $USER;
        $userid = $USER->get('id');
        require_once(get_config('libroot') . 'group.php');
        if (!group_user_access($groupid)) {
...
//End-Eshwari
    ...
    public static function get_submitted_views($groupid) {
		...
		$evalcount= count($evalstate);
		$viewdata = get_records_sql_assoc('
            SELECT v.id, v.title, v.description, v.owner, v.ownerformat, v.group, v.institution, v.submittedoutcome
            FROM {view} v 					
            WHERE v.submittedgroup = ?						
            ORDER BY v.title, v.id',
            array($groupid)
        );
	...
	//Start -Eshwari 
	 /** 
     * Get courseviews submitted to a group
     */
    public static function get_submitted_courseviews($groupid) {
        global $USER;
        $userid = $USER->get('id');
		
		$evalcount= count($evalstate);
		$viewdata = get_records_sql_assoc('
            SELECT v.id, v.title, v.description, v.owner, v.ownerformat, v.group, v.institution, v.submittedcourseoutcome
            FROM {view} v 					
            WHERE v.submittedgroup = ?						
            ORDER BY v.title, v.id',
            array($groupid)
	...
	/** 
     * Get views submitted to a group for formative assessment
     */
    public static function get_formative_views($groupid,$userid) {
        $viewdata = get_records_sql_assoc('
            SELECT v.id, v.title, v.description, v.owner, v.ownerformat, v.self_level_formative, v.self_describe_formative, v.formativecount
            FROM {view} v
            WHERE v.group = ?			
            AND v.self_describe_formative IS NOT NULL
			AND v.assessCount=0
			AND ((v.id NOT IN (SELECT DISTINCT vf.view from {view_feedback} vf where v.id=vf.view AND vf.author = ? AND TIME_TO_SEC(TIMEDIFF(vf.ctime,v.formativetime))>0))
				OR v.id NOT IN (SELECT DISTINCT vf.view from {view_feedback}vf)
				OR v.id NOT IN (SELECT DISTINCT vf.view from {view_feedback} vf where author=?))
			ORDER BY v.title, v.id',
			array($groupid,$userid,$userid)
        );
	...
	//Start-Eshwari
	public static function get_formative_courseviews($groupid,$userid) {
        $viewdata = get_records_sql_assoc('
            SELECT v.id, v.title, v.description, v.owner, v.ownerformat, v.self_level_formative, v.self_describe_formative, v.formativecount
            FROM {view} v
            WHERE v.group = ?			
	...
	//End-Eshwari
      ...
      /** 
     * Get submitted views for outcomes, suboutcomes
     */
    public static function get_submitted_views_for_outcome($groupid,$suboutcome) {
	  global $USER;
      $userid = $USER->get('id');		
	  $outrec = get_record('outcomes','id',$suboutcome);
	  if($outrec->special_access){
		...
		INNER JOIN {outcome_results} o ON (o.view_id!=v.id AND o.member = ? AND o.level_assigned!=\'final\')
            WHERE submittedgroup = ?
		AND u.primary_focus = ?
            ORDER BY title, id',
            array($userid,$groupid, $outrec->special_access)
        	);
	  }else{
        	$viewdata = get_records_sql_assoc('
            SELECT v.id, v.title, v.description, v.owner, v.ownerformat, v.group, v.institution, v.assessCount
	...
	//Start-Eshwari 
	   /** 
     * Get submitted views for courseoutcomes, subcourseoutcomes
     */
    public static function get_submitted_views_for_courseoutcome($groupid,$subcourseoutcome) {
	  global $USER;
      $userid = $USER->get('id');		
	  $outrec = get_record('courseoutcomes','id',$subcourseoutcome);
	  if($outrec->special_access){
		$viewdata = get_records_sql_assoc('
            SELECT v.id, v.title, v.description, v.owner, v.ownerformat, v.group, v.institution, v.assessCount
            FROM {view} v
		INNER JOIN {usr} u ON (u.id = v.owner)
		INNER JOIN {courseoutcome_results} o ON (o.view_id!=v.id AND o.member = ? AND o.level_assigned!=\'final\')
            WHERE submittedgroup = ?
		AND u.primary_focus = ?
            ORDER BY title, id',
            array($userid,$groupid, $outrec->special_access)
	...
	/** 
     * Get final views for outcomes, suboutcomes
     */
    public static function get_final_views_for_outcome($groupid,$suboutcome) {
	  global $USER;
      $userid = $USER->get('id');		
	  $outrec = get_record('outcomes','id',$suboutcome);
	  if($outrec->special_access){
     ...
     * Get final views for outcomes, suboutcomes
     */
    public static function get_final_views_for_courseoutcome($groupid,$subcourseoutcome) {
	  global $USER;
      $userid = $USER->get('id');		
	  $outrec = get_record('courseoutcomes','id',$subcourseoutcome);
	  if($outrec->special_access){
    ...
    public static function get_extra_view_info(&$viewdata) {
        ...
        if ($viewdata) {
            ...
            foreach ($viewdata as &$v) {
				...
				$count = get_record('view','id',$v->id);
				$evalstate = get_records_sql_array("SELECT * FROM {outcome_results} o WHERE o.member = ? AND o.view_id=? AND submitted=1", array($USER->get('id'),$v->id));				
				$view_outcomes = get_records_sql_array("SELECT id from outcomes where main_outcome=?", array($v->submittedoutcome));
				$evalcount = count($evalstate);
				$outcomecount = count($view_outcomes);								
				if(($evalcount-$outcomecount)==1)  //1 added for final level assigned record
				$v->evaluated=1;
				
				//Added by Shashank for reassessmen message to committee
				$reassess = get_records_sql_array("SELECT * FROM {outcome_results} o WHERE o.reassess = 1 AND o.view_id = ? AND o.member=?", array($v->id,$USER->get('id'))); 
				/*foreach($reassess as $r)
				{
					echo "Count is".$r->reassess;
				}*/
				if($reassess)
				$v->reassess = 1;
				//End of message add
				
				$v->status = $count->assessCount;
                if ($v->owner) {
	...
	//start-Eshwari 
	  public static function get_extra_courseview_info(&$viewdata) {
		global $USER;
        if ($viewdata) {
            // Get view owner details for display
            $owners = array();
            ...
            foreach ($viewdata as &$v) {
                $v->shortdescription = clean_html(str_shorten_html(str_replace('<br />', ' ', $v->description), 100, true));
				$count = get_record('view','id',$v->id);
				$evalstate = get_records_sql_array("SELECT * FROM {courseoutcome_results} o WHERE o.member = ? AND o.view_id=? AND submitted=1", array($USER->get('id'),$v->id));				
				$view_courseoutcomes = get_records_sql_array("SELECT id from courseoutcomes where main_courseoutcome=?", array($v->submittedcourseoutcome));
				$evalcount = count($evalstate);
				$courseoutcomecount = count($view_courseoutcomes);								
				if(($evalcount-$courseoutcomecount)==1)  //1 added for final level assigned record
				$v->evaluated=1;
				
				//Added by Shashank for reassessmen message to committee
				$reassess = get_records_sql_array("SELECT * FROM {courseoutcome_results} o WHERE o.reassess = 1 AND o.view_id = ? AND o.member=?", array($v->id,$USER->get('id'))); 
				/*foreach($reassess as $r)
				{
					echo "Count is".$r->reassess;
				}*/
	...
	//start-Eshwari
public static function get_templatesearch_data_course(&$search) {
        require_once(get_config('libroot') . 'pieforms/pieform.php');
        $results = self::view_coursesearch($search->query, $search->ownerquery, null, $search->copyableby, $search->limit, $search->offset, true);
    ...
    public function get_feedback($limit=10, $offset=0, $lastpage=false) {
        ...
        if ($feedback) {
            foreach ($feedback as &$f) {
				...
				// Start-Anusha
					$viewrec = get_record('view','id',$viewid);
					$authrec = array();
					if($viewrec->submittedoutcome){
						$userrole = get_records_sql_array('
										...
										WHERE gm.member = ?
										AND g.outcome = ?',
									array($userid,$viewrec->submittedoutcome)
								);
						if($userrole->role == "member") {
							$authrec = get_records_sql_array('
										...
										WHERE gm.member = ?
										AND g.outcome = ?',
									array($f->author,$viewrec->submittedoutcome)
								);			
						}
					}
					if($authrec){
