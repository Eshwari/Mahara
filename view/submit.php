...
$options = array();
$courseoutcomedes = get_record('courseoutcomes','id',$view->submittedcourseoutcome);
if($courseoutcomedes->eval_type == 'Level'){
	...
	'SELECT *
	  ...
	  AND rubric_no = 0
	  ORDER BY level_val',
	array($view->submittedcourseoutcome)
	);
	$options['Level'] = "Select a level";
	$var = "Level";
	foreach($courseoutcomelevels as $courseoutcomelevel){
		$options[$var.$courseoutcomelevel->level_val] = $var.$courseoutcomelevel->level_val;
	}
}else{
...
$form = pieform(array(
	...
	// Display the self assessment part
        'options' => array(
			'type' => 'select',
			'title' => 'Set a level againt the courseoutcome',
