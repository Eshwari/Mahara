...
function submitview_submit(Pieform $form, $values) {
	...
	$formativeCount = get_record('view', 'id', $viewid);	
	if($_POST['iscourseoutocome']){
		...
		update_record('view', array('formativecount' => $formativeCount->formativecount+1, 'formativetime' => db_format_timestamp(time())), array('id' => $viewid));
	}else{
