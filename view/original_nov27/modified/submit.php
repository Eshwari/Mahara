...
if($view->submittedcourseoutocome){

$options = array();
$courseoutocomedes = get_record('courseoutocomes','id',$view->submittedcourseoutocome);
if($courseoutocomedes->eval_type == 'Level'){
	...
	'SELECT *
	  ...
	  AND rubric_no = 0
	  ORDER BY level_val',
	array($view->submittedcourseoutocome)
	);
	$options['Level'] = "Select a level";
	$var = "Level";
	foreach($courseoutocomelevels as $courseoutocomelevel){
		$options[$var.$courseoutocomelevel->level_val] = $var.$courseoutocomelevel->level_val;
	}
}else{
...
$form = pieform(array(
	...
	// Display the self assessment part
        'options' => array(
			'type' => 'select',
			'title' => 'Set a level againt the courseoutocome',
...
function submitview_submit(Pieform $form, $values) {
	...
	if($_POST['iscourseoutocome']){
		update_record('view', array('submittedgroup' => $groupid,'self_level' => $_POST['options'],'self_describe' => $_POST['explain']), array('id' => $viewid));
	}else{
		update_record('view', array('submittedgroup' => $groupid,'self_level' => null,'self_describe' => null), array('id' => $viewid));
	}
    //End-Anusha
