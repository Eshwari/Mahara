...
if (!$tutorgroupdata = @get_records_sql_array('SELECT g.id, g.name
    ...
    foreach ($data->data as &$view) {
        if (empty($view['submittedto'])) {
			...
			//printf($view['submittedoutcome']);
			//Start-Anusha
			//if(!$view['submittedoutcome']){ // added by eshwari
			if(!$view['submittedoutcome'] || $view['submittedcourseoutcome'] ){
			printf('inside');
			...
			else
			{
				...
				{
				   $grouprec = get_record('group','courseoutcome',$view['submittedcourseoutcome'],'deleted',0,'id',$view['group']);
