...
$data = View::get_mycourseviews_data($limit, $offset);
//printf('heer');
$userid = $USER->get('id');
...
if (!$tutorgroupdata = @get_records_sql_array('SELECT g.id, g.name
    ...
    foreach ($data->data as &$view) {
        if (empty($view['submittedto'])) {
			...
			//Start-Anusha
			//if(!$view['submittedoutcome']){ // added by eshwari
			if(!$view['submittedcourseoutcome'] ){
			//printf('inside');
			...
			//printf ($view['submittedcourseoutcome']);
			//printf($view['group']);
			//commented as there is no feilds ouutcome in group table
			// $grouprec = get_record('group','outcome',$view['submittedoutcome'],'deleted',0,'id',$view['group']);
			   $grouprec = get_record('group','deleted',0,'id',$view['group']);
				$x =$grouprec->id;
				printf((int)$x);
				printf($grouprec->name);
				
				$tempview['elements']['subout'] = array (
						'type' => 'hidden',
						'value' => $grouprec->id,
				);
		        $tempview['elements']['courseoutcometext'] = array(
                        'type' => 'html',
            ...
            $tempview['elements']['button1'] = array(
                        'type' => 'html',                        
						'value' => '<a href=\'submitformative.php?id='.$view['id'].'&group='.$grouprec->id.'\'>'.get_string('formassess').'</a><br/>'
                    );
			$tempview['elements']['submit2'] = array(
                        'type' => 'html',
                        'value' => '<a href=\'submit.php?id='.$view['id'].'&group='.$grouprec->id.'\'>'.get_string('summassess').'</a>'
