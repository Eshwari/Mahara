...
if ($can_edit) {
  ...
  //printf('courseoffering it is');
  $data = View::get_mycourseviews_data($limit, $offset, $group->id);
  }
  else{
 // printf('outcome it is');
    $data = View::get_myviews_data($limit, $offset, $group->id);
	//added to incorporate courseoutcome-Eshwari
	}
}
/*
if ($can_edit) {
...
// member
   ...
   if ($grouprec->courseoffering){
    printf('courseoffering it isss');
	
   $data = View::view_coursesearch(null, null, (object) array('group' => $group->id), null, $limit, $offset);
   printf($group->id);
	}
	else{
	printf('outcome');
