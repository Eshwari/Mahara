...
$submittedcourseoutcome = $view->get('submittedcourseoutcome');
$admin = 0;
$finalAssessment = 0;
...
//End - Anusha
printf('before feedback block');


$isallowed = 0;
//Code start by Shashank
if ($submittedoutcome)
{
//sub outcome code
if ($submittedgroup){
...
printf($outcomedes->id);
// Start of deletion by Shashank
...
elseif ($submittedcourseoutcome) {
if ($submittedgroup){
	if(!$groupid && group_user_can_assess_submitted_views($submittedgroup, $USER->get('id'))) {  
	$isallowed = 1;
	$ismain = 1;
	}else if($grouprec && $submittedcourseoutcome && $grouprec->outcome != $submittedcourseoutcome){
	}
	//commented -Eshwari -sub courseoutcome
	/*
	else if($grouprec && $submittedcourseoutcome && $grouprec->outcome != $submittedcourseoutcome){

			$subcourseoutcome = get_record('courseoutcomes','id',$grouprec->outcome,'deleted',0);
			if(!$suboutcome){
    				throw new AccessDeniedException('Outcome does not exist');					
			}
			$temp_outcome = $suboutcome->main_outcome;
			while($temp_outcome){
				if($submittedcourseoutcome == $temp_outcome){
					$submittedgroup = $groupid;
					$submittedcourseoutcome = $grouprec->outcome;
					$isallowed = 1;
					$ismain = 0;
					$temp_outcome = "";
				}else{
...
$backgroup = $submittedgroup;
$courseoutcomedes = get_record('courseoutcomes','id',$submittedcourseoutcome,'deleted',0);
...
if($isallowed){
    ...
    'SELECT *
	   ...
	   AND level_assigned=\'final\'
       AND (submitted = 1 OR submitted = 2)',
    array($submittedcourseoutcome, $viewid)
	);
...
if ($USER->is_logged_in() && !empty($_SERVER['HTTP_REFERER'])) {
	...
	if($view->get('submittedgroup') && submittedoutcome){
		if(($_SERVER['HTTP_REFERER'] == get_config('wwwroot') . 'view/listassessments.php?id=' . $viewid .'courseoutcome=' .$submittedoutcome) || ($_SERVER['HTTP_REFERER'] == get_config('wwwroot') . 'view/showassessments.php?id=' . $viewid .'outcome=' .$submittedoutcome) || ($_SERVER['HTTP_REFERER'] == get_config('wwwroot') . 'view/finalassessment.php?id=' . $viewid .'outcome=' .$submittedoutcome)) {
			$urlback = get_config('wwwroot') . 'group/view.php?id=' . $submittedgroup;
			$smarty->assign('backurl', $urlback);
		}
	}
	//start-Eshwari
	if($view->get('submittedgroup') && submittedcourseoutcome){
		if(($_SERVER['HTTP_REFERER'] == get_config('wwwroot') . 'view/listassessments.php?id=' . $viewid .'courseoutcome=' .$submittedcourseoutcome) || ($_SERVER['HTTP_REFERER'] == get_config('wwwroot') . 'view/showassessments.php?id=' . $viewid .'courseoutcome=' .$submittedcourseoutcome) || ($_SERVER['HTTP_REFERER'] == get_config('wwwroot') . 'view/finalassessment.php?id=' . $viewid .'courseoutcome=' .$submittedcourseoutcome)) {
			$urlback = get_config('wwwroot') . 'group/view.php?id=' . $submittedgroup;
			$smarty->assign('backurl', $urlback);
		}
	}
...
//Start of formative self assessmet block by Shashank
$selfevaluationformative = pieform(array(
			'name'     => 'selfeval',
			'method'   => 'get',
			'plugintype' => 'core',
			'pluginname' => 'view',
			...
			'elements' => array(
				'selflvl' => array(
					'type'  => 'html',
					'title' => 'Assigned Level',
					'value' => $view->get('self_level_formative'),
				),
				'selfdescription' => array(
					'type'  => 'html',
					'title' => 'Explanation',
					'value' => $view->get('self_describe_formative'),
				),
			),
		));
...
$viewrecord = get_record('view','id', $viewid);
if($viewrecord->assessCount==2)
...
//Start of change by Shashank
$smarty->assign('selfevaluationformative', $selfevaluationformative);
$smarty->assign('describeformative', $view->get('self_describe_formative'));
//End of change by Shashank
...
$smarty->assign('submittedcourseoutcome',$submittedcourseoutcome);
